...
class CustomConvNeXt(nn.Module):
    def __init__(self, num_classes=None):
        ...
        # 加载基础 ConvNeXt 模型
        self.base_model = convnext_base(pretrained=True)
        ...
        if num_classes is not None:
            ...
            init.kaiming_normal_(self.base_model.classifier[-1].weight, mode='fan_out', nonlinearity='relu')
            if self.base_model.classifier[-1].bias is not None:
                self.base_model.classifier[-1].bias.data.zero_()
